            ...
            if self.Check_Ints(self.EdDob_day_Entry.get()) and self.Check_Ints(self.EdDob_month_Entry.get()) and self.Check_Ints(self.EdDob_year_Entry.get()):
                ...
                if not AskDup:
                    if not AskDupRoom:
                        ...
                        if self.isimgchange:
                                self.Patient_DB.update_patient_by_id(pid,set_fields={"image_path":self.Edit_Patient_Data["image_path"]})
    ...
    def Retreive_Stats(self):
        ...
        while cap.isOpened():
            if(self.start_model and self.Load_Stats):
                ...
                cv2.putText(Frame, f"Status: {self.Sleep_Stat,self.Eating_Stat,self.Fall_Stat,self.Gesture_Stat,self.Expression} Taken Meds: {self.TakenMeds}", (20, 30), cv2.FONT_HERSHEY_PLAIN, 1.0, (255, 0, 0), 2)
